...
def summaryPengujian():
    outfile = open('precision_3_ext.csv','w')
    outfile.write("Properties,Precision,Recall,F1Measure\n")
    for pengujian_no in range(3000,3096):
        i = 0
        sum_prec = 0
        sum_rec = 0
        sum_f1m = 0
...
def countSequence():
    outfile = open('sequence_3.csv','w')
    for pengujian_no in range(3000,3096):
        data_test_count = 0
        data_uji_count = 0
        for p in db.pengujian.find({"pengujian_no":pengujian_no}):
            data_test_count = data_test_count + p["data_test_count"]
            data_uji_count = data_uji_count + p["data_uji_count"]

            properties = p['param_uji']

        outfile.write(str(properties)+","+str(data_test_count)+","+str(data_uji_count)+"\n")
        
def cobaCountSeq():
    data_uji_count = 0
    for p in db.pengujian.find({"pengujian_no":3001}):
        print p["data_uji_count"]
        data_uji_count = data_uji_count + p["data_uji_count"]
    print data_uji_count 

#profile.run("print assign_topic(); print")
#pengujianBaruLoop()
